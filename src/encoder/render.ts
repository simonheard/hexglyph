...
export function renderCode(encoded:Encoded,opt:RenderOptions):Rendered{
  ...
  if(encoded.version===2){ctx.beginPath();cornerCoords.forEach((p,i)=>i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y));ctx.closePath();ctx.strokeStyle='#111318';ctx.lineWidth=pitch*.72;ctx.lineJoin='miter';ctx.stroke();ctx.strokeStyle='#fff';ctx.lineWidth=pitch*.16;for(let i=0;i<3;i++){const t=.38+i*.09,a=cornerCoords[1],b=cornerCoords[2];ctx.beginPath();ctx.moveTo(a.x+(b.x-a.x)*(t-.018),a.y+(b.y-a.y)*(t-.018));ctx.lineTo(a.x+(b.x-a.x)*(t+.018),a.y+(b.y-a.y)*(t+.018));ctx.stroke();}}
